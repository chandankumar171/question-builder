...
//   const type = req.params.type; // 'normal' or 'mcq'
//   const wb = XLSX.utils.book_new();

//   let headers, sampleRow, sheetName;

//   if (type === 'normal') {
//     headers = ['type', 'question', 'answer'];
//     sampleRow = ['normal', 'What is the capital of India?', 'New Delhi'];
//     sheetName = 'Normal Questions';
//   } else {
//     headers = ['type', 'question', 'option_a', 'option_b', 'option_c', 'option_d', 'correct_answer'];
//     sampleRow = ['mcq', 'What is 2 + 2?', '3', '4', '5', '6', 'B'];
...
//       const rowObj = {};
//       headers.forEach((h, idx) => { rowObj[h] = row[idx] !== undefined ? String(row[idx]).trim() : ''; });
...
// // Single unified template — user sets 'type' to 'normal' or 'mcq' per row
...
//         });
//       } else if (type === 'mcq') {
//         const opts = { A: rowObj['option_a'] || '', B: rowObj['option_b'] || '', C: rowObj['option_c'] || '', D: rowObj['option_d'] || '' };
//         const correct = rowObj['correct_answer']?.toUpperCase() || '';
//         questions.push({
//           type: 'mcq',
//           question,
